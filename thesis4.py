...
for (num_epochs, learning_rate) in settings:
    ...
    W_conv1 = tf.Variable(initializer(shape=[3, 3, 1, 8]))
    ...
    W_fc1 = tf.Variable(initializer(shape=[7 * 7 * 8, 32]))
    b_fc1 = tf.Variable(initializer(shape=[32]))
    W_fc2 = tf.Variable(initializer(shape=[32, 10]))
    b_fc2 = tf.Variable(initializer(shape=[10]))
