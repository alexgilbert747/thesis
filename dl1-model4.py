...
print('Importing data, normalising, shuffling, and creating train/test split...')
# Import data.
X_data, y_data = pets_data.gen()

# Normalise data.
X_data = (X_data - np.min(X_data))/(np.max(X_data) - np.min(X_data))

# Shuffle data.
random_ordering = np.arange(400)
np.random.shuffle(random_ordering)
X_data = X_data[random_ordering]
y_data = y_data[random_ordering]

# Create splits.
idx_train = list(range(0,160)) + list (range(200,360))
idx_test  = list(range(160,200)) + list (range(360,400))
...
for (num_epochs, learning_rate) in settings:
    ...
    W_fc1 = tf.Variable(initializer(shape=[res//8 * res//8 * 16, 512]))
    ...
    h_pool3 = tf.nn.max_pool(h_conv3, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1],
                             padding='SAME')
    # Fully-connected layers.
    h_pool3_flat = tf.reshape(h_pool3, [-1, res//8 * res//8 * 16])

    h_fc1 = tf.nn.relu(tf.matmul(h_pool3_flat, W_fc1) + b_fc1)
    y = tf.matmul(h_fc1, W_fc2) + b_fc2
    #h_fc1 = tf.nn.relu(tf.matmul(h_pool1_flat, W_fc1) + b_fc1)
    #y = tf.matmul(h_fc1, W_fc1) + b_fc1
    ...
    i, n, train_accuracy, test_accuracy = 0, 0, [], []
